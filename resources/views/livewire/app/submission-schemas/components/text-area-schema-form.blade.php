<div class="mt-8">
    <h3 class="text-lg font-medium text-gray-700">Text area input</h3>
    ...
    <form class="space-y-12 " wire:submit.prevent="submit">
        <div class="mt-6 space-y-6 sm:mt-5 sm:space-y-5">
            ...
            <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
                <x-app.form.label for="min_characters">Minimum characters</x-app.form.label>
                <div class="mt-1 flex max-w-lg rounded-md shadow-sm sm:col-span-2 sm:mt-0">
                    <x-app.form.input
                        type="number"
                        inputmode="numeric"
                        name="min_characters"
                        id="min_characters"
                        wire:model.defer="min_characters"
                    />
                    @error('min_characters')<span>ERROR WITH LABEL {{ $message }}</span>@enderror
                </div>
            </div>
            <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
                <x-app.form.label for="max_characters">Maximum characters</x-app.form.label>
                <div class="mt-1 flex max-w-lg rounded-md shadow-sm sm:col-span-2 sm:mt-0">
                    <x-app.form.input
                        type="number"
                        inputmode="numeric"
                        name="max_characters"
                        id="max_characters"
                        wire:model.defer="max_characters"
                        required
                    />
                    @error('max_characters')<span>ERROR WITH LABEL {{ $message }}</span>@enderror
                </div>
            </div>
